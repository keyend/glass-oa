<?php /*a:2:{s:81:"D:\xampp\cygwin\www\wwwroot\cloud\or.xmr.la\app\admin\view\system\user\index.html";i:1686882086;s:68:"D:\xampp\cygwin\www\wwwroot\cloud\or.xmr.la\app\admin\view\base.html";i:1688009496;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<script>
    layui.config({
        ...
        var active = {
            add: function(){
                ns.open("<?php echo url('sysUserAdd'); ?>", '添加管理员').then(() => {
                    renderTable()
                })
            },
            status: function(uid, status){
                ns.silent('<?php echo url("sysUserStatus"); ?>?id=' + uid, {status: status?1:0});
            }
        };
                ...
                table.on("tool(LAY-user-table)", function(e) {
                    if ("del" === e.event) {
                        layer.confirm('您确定要删除该管理员？', {
                            ...
                            layer.close(i),
                            ns.silent("<?php echo url('sysUserDelete'); ?>", {id: e.data.user_id}, res => {
                                if (res.code == 0) {
                        ...
                        ns.open("<?php echo url('sysUserEdit'); ?>?id=" + e.data.user_id, '编辑管理员').then(() => {
                            pageTable.reload()
                        })
                    }
                });
