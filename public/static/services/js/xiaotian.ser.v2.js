    ...
    switch(data['cmd']) {
        ...
        case 'SSO':
            layui.use('layer', function () {
                ...
                setTimeout(function () {
                    return workerTermTrigger.logout('sso');
                }, 2000);
            });
            break;
...
var workerTermTrigger = {
    logout(t) {
        $.getJSON(ns.url('service/my/logout'), {ts: t||''}, function(res) {
            console.log(res);
            0 == res.code?setTimeout(() => location.reload(), 6E2):layer.alert(res.msg)
        })
    }
};
...
$(function () {
    ...
    $("#takeCare").click(function () {
        ...
        var tkIndex = layer.confirm('您确定要接待 ' + careCustomer.customer_name + ' ？', {
            ...
            takeData.kefu_sid = kefuUser.code;
    ...
    $("#reLink").click(function() {
        ...
        $.getJSON(ns.url("service/my/switch"), function (res) {
            var _tab_html = '';
            var _user_html = '';
    ...
    $(".login-out").click(function () {
        ...
        if(servicePool.length == 0) {
            return workerTermTrigger.logout();
        }
    ...
    $("#joinBlack").click(function () {
        ...
        layer.confirm('确定将该访客加入到黑名单？', {
            ...
            $.post(ns.url('service/customer/ban'), {
                ip: $("#ipAddr").val(),
    ...
    $("#add-my-cate").click(function () {
        ...
        layer.prompt({title: '请输入分类名称', formType: 3}, function(pass, index){
            layer.close(index);
            $.post(ns.url("cate/add"), {cate_name: pass, u: seller}, function (res) {
                if (0 == res.code) {
                    layer.msg(res.msg);
                    getMyWord();
                } else {
    ...
    $(".set-status").click(function() {
        ...
        if (event != CurrentStatus) {
            $.post(ns.url('service/my/status/update'), {
                u: seller,
                status: event
            }, function(res) {
                if (0 == res.code) {
...
function initWork() {
    $('#visitor-list').html('');
    $("#chat-area").html();
    $.getJSON(ns.url("service/list"), function (res) {
        if(0 == res.code && res.data.length > 0) {
            $.each(res.data, function (k, v) {
                addUser(v);
            });
...
function initHistoryChat() {
    $('#history-list').html('');
    $("#chat-area").html();
    $.getJSON(ns.url("service/history/list"), function (res) {
        if(0 == res.code && res.data.length > 0) {
            $.each(res.data, function (k, v) {
                addHistoryUser(v);
            });
...
function initQueue() {
    $('#queue-list').html('');
    $.getJSON(ns.url("service/customer/queue"), function (res) {
        if(0 == res.code && res.data.length > 0) {
            var _html = '';
...
function showUserDetail(name, ip) {
    ...
    $.getJSON(ns.url("service/getCity"), {ip: ip}, function (res) {
        if (0 == res.code) {
    ...
    // 拉取访客详情
    $.getJSON(ns.url("service/customer/info"), {customer_id: activeUser}, function (res) {
        if (0 == res.code) {
...
function updateUserInfo() {
    ...
    $.post(ns.url('service/customer/update'), {
        customer_id: activeUser,
...
function getChatLog(uid, page, flag, bottom) {
    ...
    $.getJSON(ns.url("service/logs/list"), {uid: uid, page: page, u: seller}, function(res){
        if(0 == res.code && res.data.length > 0){
    ...
    document.getElementById('textarea').addEventListener('paste', function(e){
        ...
        if (clipboardData) {
            ...
            if (item && item.kind === 'file' && item.type.match(/^image\//i)) {
                ...
                request.open('POST', ns.url("upload/image"));
...
// 获取我的常用语
function getMyWord() {
    $.getJSON(ns.url("service/my/words"), {}, function (res) {
        if (0 == res.code) {
...
function editMyWord(obj) {
    layer.prompt({title: '编辑分类名称', formType: 3, value: $(obj).attr('data-title')},
        function(pass, index){
            layer.close(index);
            $.post(ns.url('cate/edit'), {cate_name: pass, u: seller, cate_id: $(obj).attr('data-id')},
                function (res) {
                    if (0 == res.code) {
                        layer.msg(res.msg);
                        getMyWord();
...
function delMyWord(obj) {
    layer.confirm('确定要删除该分类以及分类下的常用语？', {
        ...
        $.post(ns.url('cate/delete'), {u: seller, cate_id: $(obj).attr('data-id')}, function (res) {
            if (0 == res.code) {
                layer.msg(res.msg);
                getMyWord();
            } else {
...
function addMyWord(obj) {
    layer.open({
        type: 2
        ,title: '添加常用语'
        ,content: ns.url('service/my/words/add') + '?cate_id=' + $(obj).attr('data-id')
        ,area: ['50%', '70%']
    });
}
...
function delWord(obj) {
    layer.confirm('确定要删除该常用语？', {
        ...
        $.post(ns.url('service/my/words/delete'), {u: seller, word_id: $(obj).attr('data-id')}, function (res) {
            if (0 == res.code) {
                layer.msg(res.msg);
                getMyWord();
            } else {
