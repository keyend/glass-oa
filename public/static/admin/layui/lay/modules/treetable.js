...
layui.define("jquery", function(e) {
    ...
    var o = layui.jquery,
        ...
        i.prototype.traverseModel = function(treeTable, parentNode, item, childrenAttrs) {
            ...
            tableHeaderStr += (i.checkbox == false ? '' : '<th style="width:10px"><input type="checkbox" name="treeCheckbox[]" lay-skin="primary" lay-filter="allChoose"></th>');
            ...
            layui.each(n, function(a, n) {
                ...
                if (i.spreadable) {
                    ...
                    if (p && treeNode.parentId != 'root') {
                        p = treeNode.parent.item.spread?null:treeNode.parentId;
                    }
                }
                var l = n.children && n.children.length > 0,
                    str = o(['<tr class="' + (p ? "layui-hide layui-anim layui-anim-fadein" : "layui-anim layui-anim-fadein") + '" id="' + n.id + '">', 
                        function() {
                            if (i.checkbox){
                                return '<td><input type="checkbox" name="treeCheckbox[]" lay-skin="primary" lay-filter="*" value="' + n.id + '" ' + ((n.checked && n.checked == true) ? 'checked="checked"' : "") +'></td>';
                            }
                        }(), 
                        function() {
                            var ret = "", ts;
            ...
            f.on('checkbox(*)',function(data){
                ...
                if (options.callback && options.callback.onCheck) {
                    return options.callback.onCheck(node, nt.mapping);
                }
            }); 
                            ...
                            for (var ind = 0; ind < i.layout.length; ind++) {
                                if (i.layout[ind].treeNodes) {
                                    ret += '<td class="' + i.layout[ind].colClass + '" style="' + i.layout[ind].style + '">'
                                        ...
                                        + "</i>") + ("<cite>111" + (n.name || "未命名") + "</cite></a></li></td>");
                                } else if (i.layout[ind].render) {
                                    ret += '<td class="' + i.layout[ind].colClass + '" style="' + i.layout[ind].style + '">' + i.layout[ind].render(JSON.stringify(n), n) + '</td>'
                                } else {
                                    ret += '<td class="' + i.layout[ind].colClass + '" style="' + i.layout[ind].style + '">' + n[i.layout[ind].field] + '</td>';
            ...
            if (e.callback) {
                ...
                e.callback.done && e.callback.done(this, v)
            }
