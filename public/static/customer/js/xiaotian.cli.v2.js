...
// 统一客服不在线，留言提示语
var leaveMsgUrl = ns.url('leave_message') + "?referer=" + goback_url;
...
function staffService () {
    ...
    if (2 == type) {
        ...
        window.addEventListener('message', function(event){
            var msg = typeof(event['data']) == "string" ? event.data.length > 2 ? JSON.parse(event.data) : {} : event.data;
...
function getChatLog(uid, page, flag, bottom) {
    $.getJSON(ns.url('talk/logs'), {uid: uid, page: page, tk: tk, t: t, u: seller}, function(res){
